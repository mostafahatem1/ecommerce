                ...
                <div class="modal-body p-0" >
                    @if($productModalCount)
                        <div  class="row align-items-stretch">
                            <div class="col-lg-6 p-lg-0" wire:ignore wire:key="product-image-{{ $productModal->id }}">
                                @foreach($productImages as $index => $media)
                                    @if($index == 0)
                                        <a class="product-view d-block h-100 bg-cover bg-center"
                                           style="  width: 88%;  background: url({{ asset('backend/uploads/products/'.$media->file_name) }}?v={{ time() }})"
                                           href="{{ asset('backend/uploads/products/'.$media->file_name) }}?v={{ time() }}"
                                           data-lightbox="productview"
                                           title="{{ $productModal->name }}"></a>
                                    @else
                                        <a class="d-none"
                                           href="{{ asset('backend/uploads/products/'.$media->file_name) }}"
                            ...
                            <div  class="col-lg-6">
                                ...
                                <div   class="p-5 my-md-4">
                                    <ul wire:ignore wire:key="{{ $productModal->id }}"  class="list-inline mb-2">
                                    ...
                                    <div class="row align-items-stretch mb-4">
                                        ...
                                        <div class="col-sm-5 pl-sm-0"><a wire:click.prevent="addToCart()"
                                                class="btn btn-dark btn-sm btn-block h-100 d-flex align-items-center justify-content-center px-0"
                                                href="#">Add to cart</a></div>
                                    </div>
                                    <a wire:click="addToWishList()"  class="btn btn-link text-dark p-0" ><i class="far fa-heart mr-2"></i>Add to wish list</a>
                                </div>
                            </div>
                        </div>
                    @endif
